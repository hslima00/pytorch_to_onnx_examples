...
from lib.models import seg_hrnet
...
def main():
    ...
    hrnet = get_seg_model(cfg).to(DEVICE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        avg_loss, epoch_losses = train_function(train_loader, hrnet, optimizer, loss_function, DEVICE)
        all_losses.extend(epoch_losses)
        print(f"Training Loss: {avg_loss:.4f}")

        # Validation step
        val_loss = validate(hrnet, val_loader, loss_function, DEVICE)
        ...
        if val_loss < best_loss - EARLY_STOP_THRESHOLD:
            ...
            patience_counter = 0
            torch.save({
                'model_state_dict': hrnet.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'epoch': epoch,
                'loss': best_loss,
            }, 'best_hrnet_model.pth')
        ...
        if (epoch + 1) % 5 == 0:
            _, train_fire_iou, train_smoke_iou = test(hrnet, train_loader, loss_function, DEVICE)
            _, val_fire_iou, val_smoke_iou = test(hrnet, val_loader, loss_function, DEVICE)
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig("train_val_loss_curve_hrnet.png")
    plt.close()

    # After training, evaluate on the test set
    print("Evaluating on test set...")
    test_loss, test_fire_iou, test_smoke_iou = test(
        hrnet, test_loader, loss_function, DEVICE
    )
