...
def create_swin_transformer(num_classes):
    ...
    # Modify the head to output the desired shape
...
class CustomDataset(Dataset):
    def __init__(self, img_dir, transform=None, target_size=(224, 224)):
...
def main():
    parser = argparse.ArgumentParser(description="Train Swin Transformer model")
    parser.add_argument(
        "--dataset",
        type=str,
        default=r"/home/hslima/firefront-benchmark/deeplabv3plus/YOLODataset",
    ...
    swin_transformer = create_swin_transformer(NUM_CLASSES).to(DEVICE)
    optimizer = optim.Adam(swin_transformer.parameters(), lr=LEARNING_RATE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        avg_loss, epoch_losses = train_function(train_loader, swin_transformer, optimizer, loss_function, DEVICE)
        all_losses.extend(epoch_losses)
        print(f"Training Loss: {avg_loss:.4f}")

        # Validation step
        val_loss = validate(swin_transformer, val_loader, loss_function, DEVICE)
        ...
        if val_loss < best_loss - EARLY_STOP_THRESHOLD:
            ...
            patience_counter = 0
            torch.save({
                'model_state_dict': swin_transformer.state_dict(),
        ...
        if (epoch + 1) % 5 == 0:
            _, train_fire_iou, train_smoke_iou = test(swin_transformer, train_loader, loss_function, DEVICE)
            _, val_fire_iou, val_smoke_iou = test(swin_transformer, val_loader, loss_function, DEVICE)
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig("train_val_loss_curve_swin_transformer.png")
    plt.close()

    # After training, evaluate on the test set
    print("Evaluating on test set...")
    test_loss, test_fire_iou, test_smoke_iou = test(
        swin_transformer, test_loader, loss_function, DEVICE
    )
