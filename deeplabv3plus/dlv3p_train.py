...
def create_deeplabv3plus(num_classes):
    ...
    class WrappedDeepLabV3Plus(nn.Module):
    ...
    return WrappedDeepLabV3Plus(model)
...
def train_function(data, model, optimizer, loss_fn, device):
    ...
    for X, y in progress_bar:
        ...
        print(f"Input shape: {X.shape}, Input type: {X.dtype}")  # Add this line
        optimizer.zero_grad()
        output = model(X)
...
deeplabv3plus = create_deeplabv3plus(NUM_CLASSES).to(DEVICE)
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        avg_loss, epoch_losses = train_function(train_loader, deeplabv3plus, optimizer, loss_function, DEVICE)
        all_losses.extend(epoch_losses)
        print(f"Training Loss: {avg_loss:.4f}")

        # Validation step
        val_loss = validate(deeplabv3plus, val_loader, loss_function, DEVICE)
        ...
        if (epoch + 1) % 5 == 0:
            _, train_fire_iou, train_smoke_iou = test(deeplabv3plus, train_loader, loss_function, DEVICE)
            _, val_fire_iou, val_smoke_iou = test(deeplabv3plus, val_loader, loss_function, DEVICE)
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig("train_val_loss_curve_deeplabv3plus.png")
    plt.close()

    # After training, evaluate on the test set
    print("Evaluating on test set...")
    test_loss, test_fire_iou, test_smoke_iou = test(
        deeplabv3plus, test_loader, loss_function, DEVICE
    )
